Shahcoin Core version 0.13.2 is now available from:
  ...
  <https://download.shahcoin.org/shahcoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/shahcoin-project/shahcoin/issues>
...
ECDSA signatures inside Shahcoin transactions now use validation using
...
Previous versions of Shahcoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Shahcoin Core was to raise the default minimum
relay fee.

Shahcoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Shahcoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running shahcoind without having to do any manual configuration.
...
Shahcoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Shahcoin Core will relay transactions with
...
entirely in Shahcoin Core 0.13, and it is at this time undecided whether the
...
Shahcoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Shahcoin Core automatically creates a hidden service to listen on, without
manual configuration. Shahcoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Shahcoin Core is listening, and
...
Shahcoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Shahcoin
Core to use floating fees. By default, Shahcoin Core will use floating
fees.
...
`shahcoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /shahcoinrpc>
        ...
        # optional bypass shahcoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< shahcoinrpc:<password>
    </Location>
...
Restarting shahcoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in shahcoin.conf
...
shahcoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/shahcoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Shahcoin Core code base has
...
- `shahcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `shahcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Shahcoin scripting system that in combination with
...
Shahcoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Shahcoin Core.
...
The two phases are distinct in the Shahcoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Shahcoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Shahcoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Shahcoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of shahcoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of shahcoins being spent is exactly
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Shahcoin's current ledger (called the UTXO set),
...
- **Script versioning:** Segwit makes it easy for future soft forks to allow
  ...
  currently being researched by Bitcoin and Shahcoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Shahcoin
client to Shahcoin Core 0.13.2 to signal for segwit support. When a
...
- `shahcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `shahcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
